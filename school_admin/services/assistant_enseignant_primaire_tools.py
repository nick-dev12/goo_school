"""
Outils ORM pour l'assistant vocal enseignant primaire.
"""
import json
import logging
import re
...
from school_admin.services.assistant_enseignant_actions import (
    ...
    build_enseignant_action_tool_schemas,
)
...
def tool_get_notes_eleve(ctx, args):
    ...
    qs = NotePrimaire.objects.filter(
        eleve=eleve,
        evaluation_primaire__professeur=ctx.professeur,
...
def tool_get_notes_classe(ctx, args):
    ...
    qs = NotePrimaire.objects.filter(
        ...
        evaluation_primaire__professeur=ctx.professeur,
    ).select_related('eleve', 'evaluation_primaire')
    if eval_q:
        qs = qs.filter(evaluation_primaire__titre__icontains=eval_q)
    if ctx.annee_scolaire:
        qs = qs.filter(
            Q(annee_scolaire=ctx.annee_scolaire) | Q(annee_scolaire__isnull=True)
        )
    items = [
        {
            ...
            'evaluation': n.evaluation_primaire.titre,
            'note': _safe_decimal(n.note),
        }
...
def tool_get_eleves_difficulte(ctx, args):
    ...
    ).select_related('eleve', 'matiere', 'classe')
    if ctx.annee_scolaire:
        qs = qs.filter(annee_scolaire=ctx.annee_scolaire)
    if periode:
        qs = qs.filter(periode_scolaire=periode)
    items = [
        {
            ...
            'classe': m.classe.nom if m.classe_id else None,
            'matiere': m.matiere.nom if m.matiere_id else None,
            'moyenne': _safe_decimal(m.moyenne),
        }
        for m in qs.order_by('moyenne')[:SEARCH_LIMIT]
    ]
    return {
        'seuil': seuil,
        'nb_eleves': len(items),
        'eleves': items,
        'message': f"{len(items)} élève(s) avec moyenne inférieure à {seuil}.",
    }
...
def tool_ouvrir_classe(ctx, args):
    ...
    url = reverse('enseignant_primaire:detail_classe', args=[classe.id])
...
def tool_lister_pages(_ctx, _args):
    from school_admin.services.assistant_pages_enseignant_primaire import list_pages
...
def tool_ouvrir_page(ctx, args):
    from school_admin.services.assistant_pages_enseignant_primaire import (
        find_page,
        page_url,
        related_pages,
    )
...
ENSEIGNANT_PRIMAIRE_TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        'function': {
            'name': 'lister_pages',
            'description': 'Pages de votre espace enseignant primaire.',
            'parameters': {'type': 'object', 'properties': {}},
        },
    },
    {
...
def execute_enseignant_primaire_tool(ctx, name, arguments):
    handler = ENSEIGNANT_PRIMAIRE_TOOL_HANDLERS.get(name)
    if handler:
        try:
