/**
 * Convocations — navigation onglets + filtres (UI v2)
 */
...
(function () {
  ...
  function filterStudentsConvocation(classeId) {
  ...
  function clearSearchCertificat(classeId) {
    ...
    if (searchInput) {
      searchInput.value = '';
      filterStudentsConvocation(classeId);
    }
  }
  ...
  document.addEventListener('click', function (event) {
    var niveauBtn = event.target.closest('.cnv-niveau-tab[data-tab]');
    if (niveauBtn) {
  ...
  document.addEventListener('input', function (event) {
    if (event.target.matches('.cnv-search-input[data-classe-id]')) {
      filterStudentsConvocation(event.target.getAttribute('data-classe-id'));
    }
  });

  document.addEventListener('change', function (event) {
    if (event.target.matches('.cnv-filter-select[data-classe-id]')) {
      filterStudentsConvocation(event.target.getAttribute('data-classe-id'));
    }
  });
